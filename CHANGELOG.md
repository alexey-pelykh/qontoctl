...
- **Docs** + **Coverage manifest**: the OAuth-flow E2E suite is now documented as local-only **by design** rather than as pending a CI retrofit. The mock-OAuth-server retrofit explored in #591 — route CI OAuth-flow coverage through recorded sandbox fixtures replayed via [nock](https://github.com/nock/nock) or [msw](https://mswjs.io/) — was evaluated and **declined**, for three reasons: (1) it is structurally redundant with the existing CI unit coverage of the OAuth client (`packages/core/src/auth/oauth-service.test.ts` exercises request construction, Zod response parsing, and every HTTP error path against a stubbed `fetch`; `oauth-authorization-factory.test.ts` exercises rotation-handling — adopt / persist / preserve, readOnly mode — against a stubbed `refreshAccessToken`); (2) replaying recorded fixtures forfeits fidelity to the live Qonto sandbox, which is the OAuth-flow E2E suite's only reason to exist over those unit tests; (3) mocking the Qonto API in E2E is out of scope per the e2e-test-reliability PRD (`docs/prds/e2e-test-reliability.md` — "E2E exists precisely because mocks lied in the past"). Forward-references to a "structural retrofit tracked at #591" are removed from `docs/oauth-flow-e2e.md`, `docs/e2e-testing.md`, and the `exchangeCode` / `refreshAccessToken` notes in `packages/e2e/coverage.json` (#591).
