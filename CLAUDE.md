...
**Sandbox note:** The Qonto sandbox environment (`thirdparty-sandbox.staging.qonto.co`) accepts **both** api-key and OAuth authentication, provided the `X-Qonto-Staging-Token` header is sent. Sandbox routing is gated on the **presence of a staging token**, not on the auth type — see "Staging token" below. CI runs E2E against production with a production-scoped api-key (no staging-token), so its requests hit `thirdparty.qonto.com` directly. Local developers using a sandbox-scoped api-key + staging-token route to the sandbox host with the same code path.

**Staging token:** The `oauth.staging-token` config field (or `QONTOCTL_STAGING_TOKEN` env var) injects an `X-Qonto-Staging-Token` header into all API requests, routing them to the Qonto sandbox environment. Both api-key and OAuth auth are accepted by sandbox when this header is set; sandbox routing is determined by the presence of the staging token, not the auth type. The token field is grouped under the `oauth` section for config organization (OAuth is the primary sandbox flow), but its routing effect applies to api-key requests too. When a staging token is present, sandbox URLs are used automatically. The staging token is also sent with OAuth token exchange, refresh, and revocation requests.
