...
import { cancelTransfer } from "@qontoctl/core";
...
interface TransferCancelOptions extends GlobalOptions, WriteOptions {
  readonly yes?: true | undefined;
}

export function registerTransferCancelCommand(parent: Command): void {
  const cancel = parent
    .command("cancel <id>")
  ...
  cancel.action(async (id: string, _opts: unknown, cmd: Command) => {
    const opts = resolveGlobalOptions<TransferCancelOptions>(cmd);
    const httpClient = await createClient(opts);

    if (opts.yes !== true) {
      process.stderr.write(`About to cancel transfer ${id}. Use --yes to confirm.\n`);
      process.exitCode = 1;
      return;
    }
    ...
    if (opts.output === "json" || opts.output === "yaml") {
      ...
      process.stdout.write(`Transfer ${id} canceled.\n`);
    }
  });
}
