...
import { getIntlTransferRequirements, type IntlTransferRequirementField } from "@qontoctl/core";
import { createClient } from "../../client.js";
import { formatOutput } from "../../formatters/index.js";
import { addInheritableOptions, resolveGlobalOptions } from "../../inherited-options.js";
import type { GlobalOptions } from "../../options.js";

function toTableRow(f: IntlTransferRequirementField): Record<string, string> {
  return {
    key: f.key,
    name: f.name,
    type: f.type,
...
export function registerIntlTransferRequirementsCommand(parent: Command): void {
  ...
  requirements.action(async (id: string, _opts: unknown, cmd: Command) => {
    ...
    const result = await getIntlTransferRequirements(client, id);
